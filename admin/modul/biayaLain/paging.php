...
if (isset($_POST['go'])) {
    ...
    if ($jabatan_admin == "Korcap") {
        ...
        $query = "SELECT * FROM tb_lain_lain WHERE (nama_operator LIKE ? OR no_unit LIKE ?) AND penempatan = ? LIMIT ?, ?";
        ...
        $query = "SELECT * FROM tb_lain_lain WHERE (nama_operator LIKE ? OR no_unit LIKE ?) LIMIT ?, ?";
        $stmt = mysqli_prepare($koneksi, $query);
        $search = "%$cari%";
    ...
    if ($jabatan_admin == "Korcap") {
        // Jika jabatan Korcap, hanya tampilkan data dari cabang yang sesuai
        $query = "SELECT * FROM tb_lain_lain WHERE penempatan = ? LIMIT ?, ?";
        $stmt = mysqli_prepare($koneksi, $query);
        ...
        // Jika jabatan selain Korcap, tampilkan seluruh data
        $query = "SELECT * FROM tb_lain_lain LIMIT ?, ?";
        $stmt = mysqli_prepare($koneksi, $query);
...
<tr>
    ...
    <td><?php echo htmlspecialchars($row['keterangan_biaya']); ?></td>
    <td><?php echo htmlspecialchars($row['waktu']); ?></td>
    <td>
        <a href="?m=biayaLain&s=detail_foto&id_biaya=<?php echo $row['id_biaya']; ?>">
            <button class="btn btn-warning">View</button>
        </a>
    </td>
    <td>
        <a href="?m=biayaLain&s=export_pdf&id_biaya=<?php echo $row['id_biaya']; ?>">
            <i class="fa fa-print"></i> Print PDF
        </a>
    </td>
    <td>
        <a href="?m=biayaLain&s=hapus&id_biaya=<?php echo $row['id_biaya'];?>" 
           onclick="return confirm('Yakin Akan dihapus?')">
           <button class="btn btn-danger">Hapus</button>
        </a> | 
        <a href="?m=biayaLain&s=ubah&id_biaya=<?php echo $row['id_biaya'];?>"
